...
def callLineProcess(request):
    if request.method == 'POST':
        ...
        if pair is None or (pair.initiator_user_id != user_id and pair.subscriber_user_id != user_id):
            return HttpResponse('Not found user_id', status=400)

        if code == 'disconnect':
            # to simplify: if one of the pair disconnects, then we delete both.
            pair.delete()
    ...
    return HttpResponse('Invalid code field', status=400)
...
def processGetStatus(pair, user_id):
    ...
    if pair.status == CallLineStatus.Answered and pair.initiator_user_id == user_id:
        pair.status = CallLineStatus.Connected  # All that remains is to exchange ice routes
        pair.save()
